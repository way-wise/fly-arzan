...
import Dropcity from "../drop-dwon/Dropcity";
import DropRooms from "../drop-dwon/DropRooms";
import DropPerson from "../drop-dwon/DropPerson";
import DatePicker from "react-datepicker";
import { useGet } from "../../utils/ApiMethod";
import { debounce } from "lodash";
import { format } from "date-fns";
import { HotelContext } from "../../context/HotelContext";
import GuestDropdown from "../drop-dwon/GuestsDrop";
...
const Tab2 = () => {
  ...
  const [citySearch, setCitySearch] = useState(""); // Stores input for API call
  const [selectCity, setSelectCity] = useState(null); // Stores input for API call
  ...
  useEffect(() => {
    if (cityNamesData?.data?.length > 0) {
      setCitySuggestion(cityNamesData?.data);
    }
  }, [cityNamesData]);
  ...
  const handleChange = (e, type, id) => {
    ...
    handleInputChange(value, setSelectCity, setCitySearch, cityFetch);
  };
  const handleSelectSuggestion = (iataCode, cityName, geoCode) => {
    setSelectCity(`${cityName} (${iataCode})`);
    setSelectCityGeoCode(geoCode);
    setCitySearch(""); // Clear API search term (prevents API from running)
    setCitySuggestion([]);
  };
  const handleDateChange = (date, type) => {
    if (date) {
      const formattedDate = format(date, "yyyy-MM-dd"); // Convert to "2025-03-10" format
      if (type == "CheckIn") setCheckIn(formattedDate); // Store formatted date
      if (type == "CheckOut") setCheckOut(formattedDate); // Store formatted date
    }
  };
        ...
        <div className="Flights-Tab-body">
          <div className="Flights-Tab-form">
            <div className="Flights-Tab-input-group  more-with">
              <div className="Flights-Tab-input-group-icon">
                ...
                >
                  <g clipPath="url(#clip0_146_752)">
                    ...
                    />
                    <path
                      d="M25.0593 24.2554H24.7664V17.1753C24.7664 16.8979 24.6096 16.6442 24.3615 16.5202L22.5691 15.624V24.2554H21.1042V13.269C21.1042 13.0241 20.9819 12.7955 20.7781 12.6597L16.3835 9.72998C16.1588 9.58013 15.8698 9.56616 15.6317 9.6936C15.3936 9.82104 15.2449 10.0692 15.2449 10.3394V24.2554H13.78V10.3394C13.78 9.52749 14.2247 8.78516 14.9405 8.40205C15.2583 8.23198 15.6168 8.14209 15.9772 8.14209C16.4126 8.14209 16.8342 8.26973 17.1964 8.51133L17.9304 9.00068V4.87061C17.9304 4.46611 17.6025 4.13818 17.198 4.13818H9.3855C8.98101 4.13818 8.65308 4.46611 8.65308 4.87061V5.70068H9.87378C11.0854 5.70068 12.071 6.68638 12.071 7.89795V24.2554H10.6062V7.89795C10.6062 7.49346 10.2783 7.16553 9.87378 7.16553H5.47925C5.07476 7.16553 4.74683 7.49346 4.74683 7.89795V17.6636H6.45581C7.66738 17.6636 8.65308 18.6493 8.65308 19.8608V24.2554H7.18823V19.8608C7.18823 19.4563 6.8603 19.1284 6.45581 19.1284H2.54956C2.14507 19.1284 1.81714 19.4563 1.81714 19.8608V24.2554H1.52417C1.11968 24.2554 0.791748 24.5833 0.791748 24.9878C0.791748 25.3923 1.11968 25.7202 1.52417 25.7202H25.0593C25.4638 25.7202 25.7917 25.3923 25.7917 24.9878C25.7917 24.5833 25.4638 24.2554 25.0593 24.2554ZM17.4421 15.7104C17.4421 15.306 17.7701 14.978 18.1746 14.978C18.5791 14.978 18.907 15.306 18.907 15.7104V17.1753C18.907 17.5798 18.5791 17.9077 18.1746 17.9077C17.7701 17.9077 17.4421 17.5798 17.4421 17.1753V15.7104ZM17.4421 20.105C17.4421 19.7005 17.7701 19.3726 18.1746 19.3726C18.5791 19.3726 18.907 19.7005 18.907 20.105V21.5698C18.907 21.9743 18.5791 22.3022 18.1746 22.3022C17.7701 22.3022 17.4421 21.9743 17.4421 21.5698V20.105ZM8.40894 15.7104C8.40894 16.1149 8.08101 16.4429 7.67651 16.4429C7.27202 16.4429 6.94409 16.1149 6.94409 15.7104V14.2456C6.94409 13.8411 7.27202 13.5132 7.67651 13.5132C8.08101 13.5132 8.40894 13.8411 8.40894 14.2456V15.7104ZM8.40894 11.3159C8.40894 11.7204 8.08101 12.0483 7.67651 12.0483C7.27202 12.0483 6.94409 11.7204 6.94409 11.3159V9.85107C6.94409 9.44658 7.27202 9.11865 7.67651 9.11865C8.08101 9.11865 8.40894 9.44658 8.40894 9.85107V11.3159ZM3.77026 22.5464C3.77026 22.1419 4.09819 21.814 4.50269 21.814C4.90718 21.814 5.23511 22.1419 5.23511 22.5464V24.2554H3.77026V22.5464Z"
                      fill="#353978"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_146_752">
                      <rect
                        ...
                        height="25"
                        fill="white"
                        transform="translate(0.791748 0.720215)"
                      />
                    </clipPath>
                  </defs>
                </svg>
              ...
              {citySuggestion?.length > 0 && citySearch !== "" && (
            ...
            <div className="Flights-Tab-input-group more-with">
              <div className="Flights-Tab-input-group-icon">
                <svg
                  ...
                  fill="none"
                >
                  <g clipPath="url(#clip0_146_698)">
                    <path
                      d="M21.9354 1.70264H19.7295V3.90852C19.7295 4.3497 19.3619 4.64381 18.9942 4.64381C18.6266 4.64381 18.2589 4.3497 18.2589 3.90852V1.70264H6.49424V3.90852C6.49424 4.3497 6.12659 4.64381 5.75895 4.64381C5.3913 4.64381 5.02365 4.3497 5.02365 3.90852V1.70264H2.81777C1.71483 1.70264 0.906006 2.65852 0.906006 3.90852V6.55558H24.4354V3.90852C24.4354 2.65852 23.1119 1.70264 21.9354 1.70264ZM0.906006 8.0997V21.5556C0.906006 22.8791 1.71483 23.7615 2.8913 23.7615H22.0089C23.1854 23.7615 24.5089 22.8056 24.5089 21.5556V8.0997H0.906006ZM7.45012 20.4526H5.68542C5.3913 20.4526 5.09718 20.232 5.09718 19.8644V18.0262C5.09718 17.732 5.31777 17.4379 5.68542 17.4379H7.52365C7.81777 17.4379 8.11189 17.6585 8.11189 18.0262V19.8644C8.03836 20.232 7.81777 20.4526 7.45012 20.4526ZM7.45012 13.835H5.68542C5.3913 13.835 5.09718 13.6144 5.09718 13.2468V11.4085C5.09718 11.1144 5.31777 10.8203 5.68542 10.8203H7.52365C7.81777 10.8203 8.11189 11.0409 8.11189 11.4085V13.2468C8.03836 13.6144 7.81777 13.835 7.45012 13.835ZM13.3325 20.4526H11.4942C11.2001 20.4526 10.906 20.232 10.906 19.8644V18.0262C10.906 17.732 11.1266 17.4379 11.4942 17.4379H13.3325C13.6266 17.4379 13.9207 17.6585 13.9207 18.0262V19.8644C13.9207 20.232 13.7001 20.4526 13.3325 20.4526ZM13.3325 13.835H11.4942C11.2001 13.835 10.906 13.6144 10.906 13.2468V11.4085C10.906 11.1144 11.1266 10.8203 11.4942 10.8203H13.3325C13.6266 10.8203 13.9207 11.0409 13.9207 11.4085V13.2468C13.9207 13.6144 13.7001 13.835 13.3325 13.835ZM19.2148 20.4526H17.3766C17.0825 20.4526 16.7884 20.232 16.7884 19.8644V18.0262C16.7884 17.732 17.0089 17.4379 17.3766 17.4379H19.2148C19.5089 17.4379 19.8031 17.6585 19.8031 18.0262V19.8644C19.8031 20.232 19.5825 20.4526 19.2148 20.4526ZM19.2148 13.835H17.3766C17.0825 13.835 16.7884 13.6144 16.7884 13.2468V11.4085C16.7884 11.1144 17.0089 10.8203 17.3766 10.8203H19.2148C19.5089 10.8203 19.8031 11.0409 19.8031 11.4085V13.2468C19.8031 13.6144 19.5825 13.835 19.2148 13.835Z"
                      fill="#353978"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_146_698">
                      <rect
                        width="25"
                        height="25"
                        fill="white"
                        transform="translate(0.170654 0.231934)"
                      />
                    </clipPath>
                  </defs>
                </svg>
              ...
              {/* <div className="form__group field one-box-add">
                <label className={`form__label more-label-x ${CheckIn ? "hidden" : ""}`} htmlFor="date">
                  Check In
                </label>
                <input
                  type="date"
            ...
            <div className="Flights-Tab-input-group more-with">
              ...
              {/* <div className="form__group field one-box-add">
                <label className={`form__label more-label-x ${CheckOut ? "hidden" : ""}`} htmlFor="date">
                  Check Out
                </label>
                <input
                  type="date"
            ...
            {/* <div className="Flights-Tab-input-group ">
              <div className="Flights-Tab-input-group-icon">
                <svg
                  ...
                  fill="none"
                >
                  <g clipPath="url(#clip0_146_712)">
                    <path
                      d="M0.490234 20.1644V21.7334C0.490234 22.1379 0.818164 22.4659 1.22266 22.4659C1.62715 22.4659 1.95508 22.1379 1.95508 21.7334V20.8968H24.0254V21.7334C24.0254 22.1379 24.3533 22.4659 24.7578 22.4659C25.1623 22.4659 25.4902 22.1379 25.4902 21.7334V20.1644V17.7588H0.490234V20.1644Z"
                      fill="#353978"
                    />
                    <path
                      d="M22.7966 11.5869H3.18389C1.69863 11.5869 0.490234 12.7953 0.490234 14.2806V16.2939H25.4902V14.2806C25.4902 12.7953 24.2818 11.5869 22.7966 11.5869Z"
                      fill="#353978"
                    />
                    <path
                      d="M10.2445 8.34473C9.56689 8.34473 9.01562 8.89595 9.01562 9.57354V10.122H16.9648V9.57354C16.9648 8.89595 16.4136 8.34473 15.736 8.34473H10.2445Z"
                      fill="#353978"
                    />
                    <path
                      d="M7.55076 9.57339C7.55076 8.08809 8.75916 6.87974 10.2445 6.87974H15.7361C17.2214 6.87974 18.4297 8.08809 18.4297 9.57339V10.1219H22.3523V6.43535C22.3523 4.95005 21.1439 3.7417 19.6585 3.7417H6.32195C4.83665 3.7417 3.6283 4.95005 3.6283 6.43535V10.1219H7.55076V9.57339Z"
                      fill="#353978"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_146_712">
                      <rect
                        width="25"
                        height="25"
                        fill="white"
                        transform="translate(0.490234 0.603516)"
                      />
                    </clipPath>
                  </defs>
                </svg>
            ...
            <div className="Flights-Tab-input-group ">
              <div className="Flights-Tab-input-group-icon">
                ...
                >
                  <path
                    d="M13.3417 12.3223C16.362 12.3223 18.8104 9.87382 18.8104 6.85352C18.8104 3.83321 16.362 1.38477 13.3417 1.38477C10.3214 1.38477 7.87292 3.83321 7.87292 6.85352C7.87292 9.87382 10.3214 12.3223 13.3417 12.3223Z"
                    fill="#353978"
                  />
                  <path
                    d="M17.2245 13.8848H9.45886C7.79568 13.8868 6.20121 14.5484 5.02516 15.7245C3.84911 16.9005 3.18749 18.495 3.18542 20.1582V24.041C3.18542 24.2482 3.26773 24.4469 3.41425 24.5934C3.56076 24.74 3.75947 24.8223 3.96667 24.8223H22.7167C22.9239 24.8223 23.1226 24.74 23.2691 24.5934C23.4156 24.4469 23.4979 24.2482 23.4979 24.041V20.1582C23.4959 18.495 22.8342 16.9005 21.6582 15.7245C20.4821 14.5484 18.8877 13.8868 17.2245 13.8848Z"
                    fill="#353978"
                  />
                </svg>
              </div>
