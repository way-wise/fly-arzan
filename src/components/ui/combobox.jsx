...
const ComboboxOptions = ({ children, ...props }) => {
  return (
    <ComboboxOptionsPrimitive
      ...
      transition
      className={cn(
        "tw:origin-top tw:mt-2 tw:border tw:border-muted tw:transition tw:duration-200 tw:ease-out tw:empty:invisible tw:data-closed:scale-95 tw:data-closed:opacity-0 tw:bg-white tw:shadow-lg tw:rounded-lg tw:z-50 tw:p-3 tw:!max-h-[280px]",
        props.className
      )}
    >
      {children}
